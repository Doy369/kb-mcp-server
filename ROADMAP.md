...
| 平均关键词召回 | 1.000 |
| 平均置信度 | 0.443 |
| 时延 p50 / p95 | 22ms / 434ms（首问含建图） |
| 平均图谱路径 | 4.55 条 |

### 已知未通过 2 例（已定位根因，非随机波动）
- `P0 故障多久响应` / `P2 问题多久响应` —— 误含另一档位的时限。
- **根因**：`samples/sla说明.md` 把三档 SLA 写在**同一 chunk 的一行**（`P0 15 分钟，P1 1 小时，P2 4 小时`），
  模板合成整段返回，导致答 P0 时必然也带上 P2 的「4 小时」。
- **修复路径**（二选一，均已在上方待办中）：
  - P2-8：结构化政策文档改用**细粒度分块**（按档位拆条）；
  - P0-1：接 LLM 后做**答案级抽取**，只答被问到的那一档。
- 保留这两个失败作为回归哨兵：修好后通过率应回到 100%。

> 评测器已支持**反向断言** `expect_not_contains`（防止召回错误分块）。
