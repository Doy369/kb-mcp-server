...
### 已知未通过 1 例（根因 = P0-3 嵌入能力，已用对照实验证实）
- `P2 问题多久响应` —— 未命中「4 小时」且误含「15 分钟」。
- **实测**：问 P2，top-1 召回了 **P0** 的 chunk（`工单响应：P0 紧急 15 分钟内响应`）。
- **关键对照**：语料对齐**前后**，这三档的文本几乎没变（只是术语统一），
  但 top-1 从「P2 4 小时」翻转成了「P0 15 分钟」——
  说明 dev 哈希嵌入对 `P0/P1/P2` 这类**字面高度相似**的片段**区分不了，排序近乎随机**。
  这是支持 **P0-3 换 bge 真实嵌入**最直接的证据。
- **修复路径**：
