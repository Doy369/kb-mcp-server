...
class IngestionPipeline:
    """摄取管线：文档切片 -> 本地嵌入(bge-zh) -> 写入向量库。"""

    def __init__(self, store: VectorStore | None = None, embedder=None):
        self.store = store or get_store()
        self.embedder = embedder or get_embedder()

    def ingest_text(self, doc_id: str, text: str, chunk_size: int = 300, overlap: int = 50) -> int:
    ...
    def ingest_file(self, doc_id: str, filename: str, data: bytes, chunk_size: int = 300, overlap: int = 50) -> int:
        """解析文件字节并按 document 粒度入库（一个文件 = 一个 doc_id）。"""
        text = extract_text(filename, data)
        return self.ingest_text(doc_id, text, chunk_size, overlap)
